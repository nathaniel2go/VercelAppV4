...
export default function Portfolio() {
  ...
  const [imageDimensions, setImageDimensions] = React.useState<{ [key: string]: { width: number; height: number } }>({});
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="w-full py-6 px-8 border-b border-black/10 dark:border-white/10 flex items-center justify-between">
        ...
        <span className="text-sm text-foreground/60">Explore my work</span>
      </header>
      ...
      <main className="w-full px-8 py-12">
        <div className="max-w-7xl mx-auto">
          ...
          <div 
            ref={containerRef}
            className="columns-3 gap-6 space-y-6"
          >
      ...
      {/* Footer */}
      <footer className="w-full py-8 px-8 border-t border-black/10 dark:border-white/10 mt-20">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-foreground/60">
            &copy; {new Date().getFullYear()} Photography Portfolio &mdash; Crafted with passion
          </p>
        </div>
      </footer>
    </div>
